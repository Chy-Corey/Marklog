...
> **一站式个人博客网站：写博客，只需要写 Markdown。** 不需要数据库，不需要后台操作，编辑 `md` 文件即可发布内容。
...
🔗 **Demo**: [http://lemonparty.com.cn](http://lemonparty.com.cn)
...
管理后台功能：
- **文章管理** — 创建、编辑、删除文章，支持上传 .md 文件，支持搜索和标签筛选
- **项目管理** — 管理项目展示，配置 GitHub/Demo 链接
- **友链管理** — 管理友情链接
- **Home 配置** — 编辑主页配置（home.md）
- **图片上传** — 上传图片到服务器
- **SQL 查询** — 执行只读 SQL 查询（仅 SELECT）
...
- **主页模块** — Hero 介绍、工作经历、项目卡片、博客预览、访问统计
- **博客系统** — 列表页（搜索 + 标签过滤）、详情页（Markdown 渲染）
- **标签聚合** — 按标签筛选文章，独立标签页
- **项目展示** — 卡片式布局，GitHub + Demo 链接，标签分类
- **友链页面** — 友情链接卡片网格，头像加载失败时首字母占位
- **图片灯箱** — 点击图片放大查看，ESC/背景关闭，滚动锁定
- **访问统计** — 记录首页访问量，按周/月统计，同一会话只记录一次
- **管理后台** — 文章/项目/友链 CRUD、Home 配置编辑、.md 文件上传、图片上传、SQL 查询
